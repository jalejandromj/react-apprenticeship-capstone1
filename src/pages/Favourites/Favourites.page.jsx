...
function FavouritesPage() {
  ...
  useEffect(() => {
    ...
    if(storedFavourites){ //If there is ANY favourite video...
  ...
  return (
    <section data-testid="favouritesPage" className="section favouritespage" ref={sectionRef}>
      <Container fluid>
        <Row>
          <Col sm={12}><h2>Favourites</h2></Col>
        </Row>
        {videos ? (
          <Row>
            {videos.map((vid) =>
              <Col key={vid.etag} sm={12} md={3} lg=  {4} style={{padding: "20px 10px"}}>
                <VideoCard name={vid.snippet.title} 
                           channel={vid.snippet.channelTitle} 
                           description={vid.snippet.description} 
                           img={vid.snippet.thumbnails.high.url}
                           mini={false}
                           onClick={(e) => history.push(`/favourites/watch-video/${vid.id}`)}
                />
              </Col>
            )}
          </Row>

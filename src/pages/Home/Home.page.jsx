...
function HomePage() {
  ...
  return (
    <section data-testid="homePage" className="section homepage" ref={sectionRef}>
      <Container fluid>
        <Row>
          <Col sm={12}>Welcome and wonderful {day}! Enjoy your {userContextVal.search} videos...</Col>
        </Row>
        {videos ? (
          <Row>
            {videos.map((vid) =>
              <Col key={vid.etag} sm={12} md={3} lg=  {4} style={{padding: "20px 10px"}}>
                <VideoCard name={vid.snippet.title} 
                           ...
                           mini={false}
                           onClick={(e) => history.push(`/watch-video/${vid.id.videoId}`)}
                />
              </Col>
            )}
          </Row>
